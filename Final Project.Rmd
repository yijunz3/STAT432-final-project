...
```{r}
  ...
  eval <- function(data_x, data_y, folds) {
    auc_list = c()
    for (i in 1:3) {
      test_idxes <- which(folds==i, arr.ind=TRUE)
      test_x <- data_x[test_idxes,]
      test_y <- data_y[test_idxes]
      train_x <- data_x[-test_idxes,]
      train_y <- data_y[-test_idxes]
      
      rf.fit = randomForest(train_x, train_y, ntree = 1000, 
                        mtry = 7, nodesize = 100, sampsize = 300)
      pred = predict(rf.fit, test_x)
      roc <- prediction(as.numeric(pred), as.numeric(test_y))
...
```{r}
  ...
  pr_auc = eval(pr_data$x, pr_data$y, fold_id_list[pr_data$idxes])
  pr_auc
  er_auc = eval(er_data$x, er_data$y, fold_id_list[er_data$idxes])
  er_auc
  her_auc = eval(her_data$x, her_data$y, fold_id_list[her_data$idxes])
  her_auc
  hist_auc = eval(hist_data$x, hist_data$y, fold_id_list[hist_data$idxes])
  hist_auc
